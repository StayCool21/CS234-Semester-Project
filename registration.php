...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $adminUsername = 'admin';  // Set your desired admin username
    $adminSql = 'SELECT * FROM registration WHERE username = ?';
    $adminStatement = $pdo->prepare($adminSql);
    $adminStatement->execute([$adminUsername]);

    if (!$adminStatement->fetch()) {
        // Admin account doesn't exist, insert it
        $adminSql = 'INSERT INTO registration (username, password) VALUES (:username, :password)';
        $adminStatement = $pdo->prepare($adminSql);

        $adminHashedPassword = password_hash('admin', PASSWORD_BCRYPT);

        $adminStatement->bindParam(':username', $adminUsername);
        $adminStatement->bindParam(':password', $adminHashedPassword);
    ...
    if ($result == 'nonexistent' && check_password($password)) {
        ...
        // now redirect to home page for registered users
